...
export default function ServiceManager() {
      ...
      switch (action.type) {
        case "ADD":
          setServices((prevServices) =>
            prevServices.filter((s) => s._id !== action.service._id)
          );
          break;
        case "EDIT":
        case "TOGGLE":
          if (action.prevState) {
            setServices((prevServices) =>
              prevServices.map((s) =>
                s._id === action.service._id ? action.prevState : s
              )
            );
          }
          break;
    ...
    if (historyIndex < history.length - 1) {
      ...
      switch (action.type) {
        ...
        case "TOGGLE":
          setServices((prevServices) =>
            prevServices.map((s) =>
              s._id === action.service._id ? action.service : s
            )
          );
          break;
        case "DELETE":
  ...
  const toggleServiceActive = (service) => {
    ...
    toast({
      ...
      description: `"${service.name}" is now ${
        updatedService.isActive ? "active" : "inactive"
      }`,
      variant: "default",
    });
  };
  ...
  const deleteService = () => {
    if (serviceToDelete) {
      ...
      setServices((prevServices) =>
        prevServices.filter((s) => s._id !== serviceToDelete._id)
      );
  ...
  const addNewService = () => {
    if (newServiceName.trim()) {
      ...
      // Update services
      setServices((prevServices) =>
        sortServices([...prevServices, newService])
      );
  ...
  const handleDragEnd = (result) => {
    ...
    const reorderedActiveServices = activeServicesCopy.map(
      (service, index) => ({
        ...service,
        order: index + 1,
      })
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Manage, organize, and prioritize your services
            </p>
          </div>
          ...
          <div className="space-y-6">
            <Card className="overflow-hidden border shadow-sm">
              <CardHeader className="bg-background pb-3">
                ...
                <CardDescription>
                  Drag to reorder. Active services are displayed to users.
                </CardDescription>
              </CardHeader>
            ...
            <div>
              {unsavedChanges && (
                <div className="text-sm text-muted-foreground">
                  <span className="font-medium">{changesMade} changes</span>{" "}
                  pending
                </div>
              )}
            </div>
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will remove "{serviceToDelete?.name}" from your services
