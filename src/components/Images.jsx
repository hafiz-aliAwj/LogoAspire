...
export default function ImageManagement() {
  ...
  const fetchImages = async (serviceName) => {
    setLoadingImages(true)
    try {
      // Simulating API call - replace with actual API
      // const response = await fetch(`/api/images?service=${serviceName}`)
      // if (!response.ok) throw new Error("Failed to fetch images")
      // const data = await response.json()

      // Mock data for demonstration
      const response = await fetch(`/api/images?category=${serviceName}`);
      ...
      const serviceImgs = data.sort((a, b) => a.orderAtPage - b.orderAtPage);
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      setHistoryIndex(-1)
      setChanges([])
      fetchImages(manageService) // Reset to original state
    }
  }
  ...
  const applyChangesToUI = (changesArray) => {
    ...
    changesArray.forEach((change) => {
      if (change.type === "toggleHome") {
        ...
        setHomeImages((prev) => {
          if (data.newValue) {
            ...
            if (image && !prev.some((img) => img._id === imageId)) {
              return [...prev, { ...image, atHome: true, orderAtHome: prev.length }]
            }
          } else {
            // Remove from home
        ...
        setServiceImages((prev) => prev.map((img) => (img._id === imageId ? { ...img, atHome: data.newValue } : img)))
      } else if (change.type === "delete") {
        setHomeImages((prev) => prev.filter((img) => img._id !== change.imageId))
        setServiceImages((prev) => prev.filter((img) => img._id !== change.imageId))
  ...
  const handleToggleHome = (imageId, currentValue) => {
    ...
    if (!currentValue) {
      ...
      const image = serviceImages.find((img) => img._id === imageId)
      if (image) {
        setHomeImages((prev) => [...prev, { ...image, atHome: true, orderAtHome: prev.length }])
      }
    } else {
      // Remove from home
    ...
    setServiceImages((prev) => prev.map((img) => (img._id === imageId ? { ...img, atHome: !currentValue } : img)))
  }
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      const promises = Object.entries(changesByImage).map(async ([imageId, imageChanges]) => {
        ...
        if (imageChanges.some((c) => c.type === "delete")) {
          ...
          const toggleChanges = imageChanges.filter((c) => c.type === "toggleHome")
          let finalAtHome = serviceImages.find((img) => img._id === imageId)?.atHome || false

          if (toggleChanges.length > 0) {
            // Use the latest toggle change
            finalAtHome = toggleChanges[toggleChanges.length - 1].data.newValue
          }

          // Get final order
          const homeImage = homeImages.find((img) => img._id === imageId)
          const serviceImage = serviceImages.find((img) => img._id === imageId)
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>Upload and manage images for your services</CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) => handleInputChange("atHome", checked)}
                    />
                  ...
                  >
                    ...
                    {previewUrl ? (
                      ...
                      <div className="flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-sm font-medium">Drag and drop an image, paste, or click to browse</p>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                {loading ? (
                  ...
                  <Select value={manageService} onValueChange={setManageService}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select a service" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <Tabs defaultValue="tab1" value={manageSubTab} onValueChange={setManageSubTab}>
                <TabsList className="grid w-full grid-cols-2 mb-6">
                ...
                <TabsContent value="tab1">
                  {loadingImages ? (
                    ...
                    <DragDropContext onDragEnd={handleDragEnd}>
                      <Droppable droppableId="home-images" direction="horizontal">
                        {(provided) => (
                          <div
                              ...
                              <Draggable key={image._id} draggableId={image._id} index={index}>
                                {(provided) => (
                                  <div
                                    ...
                                    onClick={() => openSlideshow(homeImages, index)}
                                  >
                                    <div className="aspect-square overflow-hidden border-r border-b border-border">
                                      {/* eslint-disable-next-line @next/next/no-img-element */}
                                      <img
                                        src={`/api/images/getImage?filename=${image.filename}` || "/placeholder.svg"}
                                        alt={image.filename}
                ...
                <TabsContent value="tab2">
                  {loadingImages ? (
                    ...
                    <div className="text-center py-12 text-muted-foreground">No images found for this service.</div>
                  ) : (
                    <DragDropContext onDragEnd={handleDragEnd}>
                      <Droppable droppableId="service-images" direction="horizontal">
                        {(provided) => (
                          <div
                            className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-0"
                            {...provided.droppableProps}
                              ...
                              <Draggable key={image._id} draggableId={image._id} index={index}>
                                {(provided) => (
                                  <div
                                    ...
                                    {...provided.dragHandleProps}
                                    className="relative group cursor-move"
                                    onClick={() => openSlideshow(serviceImages, index)}
                                  >
                                    <div className="aspect-square overflow-hidden border-r border-b border-border">
                                      {/* eslint-disable-next-line @next/next/no-img-element */}
                                      <img
                                      ...
                                      <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                          <DropdownMenuItem
                                            onClick={(e) => {
                                              e.stopPropagation()
                                              handleToggleHome(image._id, image.atHome)
                                            }}
                                          >
                                            <Home className="h-4 w-4 mr-2" />
                                            {image.atHome ? "Remove from Home" : "Add to Home"}
                                          </DropdownMenuItem>
                                          <DropdownMenuItem
                                            onClick={(e) => {
                                              e.stopPropagation()
      ...
      {/* Slideshow Modal */}
      <Dialog open={slideshow.open} onOpenChange={(open) => setSlideshow((prev) => ({ ...prev, open }))}>
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-center p-4 h-[80vh]">
              {slideshow.images.length > 0 && (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img 
                src={`/api/images/getImage?filename=${slideshow.images[slideshow.index].filename}` || "/placeholder.svg"}
